...
impl EvmContract {
    ...
    pub fn complete_raffle(&self, raffle_id: RaffleId, attest_key: &[u8; 32]) -> Result<Option<Vec<u8>>, RaffleDrawError>  {
        self.do_action(ActionSecondary::CompleteRaffle(raffle_id), attest_key)
    }

    pub fn send_raffle_result(&self, raffle_id: RaffleId, has_winner: bool, winners: Vec<AccountId20>, attest_key: &[u8; 32]) -> Result<Option<Vec<u8>>, RaffleDrawError>  {
        self.do_action(ActionSecondary::SetWinners(raffle_id, has_winner, winners), attest_key)
    }

    pub fn do_action(&self, action: ActionSecondary, attest_key: &[u8; 32]) -> Result<Option<Vec<u8>>, RaffleDrawError>  {
...
fn decode_request(raw_data: &[u8]) -> Result<LottoRequestMessage, RaffleDrawError> {
    ...
    if request_type == 1 { // CHECK_WINNERS
        ...
        let numbers: Vec<Number> = numbers.into_iter().map(|n: &ethabi::Token| { if let ethabi::Token::Uint(v) = n { v.as_u128() as Number } else { 0 } }).collect();
...
mod tests {
    ...
    fn decode_message_draw_numbers() {
        ...
        let message = super::decode_request(encoded_message.as_slice()).expect("Error to decode message");
